...
<div class="app">
    <div class="header">
        ...
        <div class="user-info">
            ...
            <button class="btn-icon" id="settingsBtn" style="display:none;">⚙️ Настройки</button>
...
<div id="settingsModal" class="modal hidden"><div class="modal-content"><div id="settingsContent"></div><button id="closeSettings" style="margin-top:15px;">Закрыть</button></div></div>
...
const starsDisplay=document.getElementById('starsDisplay'),profileBtn=document.getElementById('profileBtn'),adminBtn=document.getElementById('adminBtn'),settingsBtn=document.getElementById('settingsBtn');
...
async function loadDialogs(){const res=await fetch('/api/search?q=');const data=await res.json();chatsListDiv.innerHTML='';data.results.forEach(u=>{let div=document.createElement('div');div.className='contact-item';div.innerHTML=`<div class="avatar">${(u.display_name||u.username)[0].toUpperCase()}</div><div><strong>${u.display_name||u.username}</strong><div>@${u.username}</div><div>${u.status==='online'?'🟢 онлайн':'⚫ офлайн'}</div></div>`;div.onclick=()=>{currentChat={type:'private',id:Math.min(currentUser.id,u.id),recipient_id:u.id,name:u.username};currentChatHeader.innerText=`Чат с ${u.username}`;loadMessages('private',currentChat.id);};chatsListDiv.appendChild(div);});}
...
async function showSettings(){const res=await fetch(`/api/user/${currentUser.username}`);const data=await res.json();document.getElementById('settingsContent').innerHTML=`
    <h3>Настройки профиля</h3>
    <label>Имя (отображаемое)</label>
    <input type="text" id="settingsDisplayName" value="${escapeHtml(data.display_name||'')}">
    <label>Описание</label>
    <textarea id="settingsBio" rows="3">${escapeHtml(data.bio||'')}</textarea>
...
`;document.getElementById('settingsModal').style.display='flex';document.getElementById('saveSettingsBtn').onclick=async()=>{const newDisplayName=document.getElementById('settingsDisplayName').value;const newBio=document.getElementById('settingsBio').value;const newPassword=document.getElementById('settingsNewPassword').value;const payload={display_name:newDisplayName,bio:newBio};if(newPassword)payload.password=newPassword;const resp=await fetch('/api/user/update_profile',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});if(resp.ok){showToast('Профиль обновлён');if(newPassword){showToast('Пароль изменён, войдите заново');logoutBtn.click();}else{document.getElementById('settingsModal').style.display='none';showProfile(currentUser.username);}}else{const err=await resp.json();showToast(err.error);}};}
function initSocket(){socket=io();socket.on('connect',()=>console.log('Socket connected'));socket.on('new_private_message',(data)=>{if(currentChat&&currentChat.type==='private'&&currentChat.id===data.chat_id){appendMessage({content:data.message,timestamp:data.timestamp,sender_id:data.from,sender_name:data.sender_name,id:data.message_id},false);}loadDialogs();});socket.emit('register');}
searchInput.addEventListener('input',()=>{clearTimeout(searchTimeout);const q=searchInput.value.trim();if(q.length<2)return;searchTimeout=setTimeout(async()=>{const res=await fetch(`/api/search?q=${encodeURIComponent(q)}`);const data=await res.json();chatsListDiv.innerHTML='';data.results.forEach(u=>{let div=document.createElement('div');div.className='contact-item';div.innerHTML=`<div class="avatar">${(u.display_name||u.username)[0].toUpperCase()}</div><div><strong>${u.display_name||u.username}</strong><div>@${u.username}</div></div>`;div.onclick=()=>{currentChat={type:'private',id:Math.min(currentUser.id,u.id),recipient_id:u.id,name:u.username};currentChatHeader.innerText=`Чат с ${u.username}`;loadMessages('private',currentChat.id);};chatsListDiv.appendChild(div);});},300);});
loginBtn.onclick=async()=>{const login=document.getElementById('loginInput').value;const pwd=document.getElementById('passwordInput').value;const res=await fetch('/api/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({login,password:pwd})});const data=await res.json();if(res.ok){currentUser=data.user;sessionStorage.setItem('user',JSON.stringify(currentUser));initSocket();authScreen.classList.add('hidden');mainScreen.classList.remove('hidden');logoutBtn.style.display='inline-block';profileBtn.style.display='inline-block';settingsBtn.style.display='inline-block';starsDisplay.style.display='inline-block';updateStars(currentUser.stars);if(currentUser.is_admin||currentUser.username==='asdfg')adminBtn.style.display='inline-block';loadDialogs();loadGifts();loadSaved();}else{showToast(data.error);}};
...
def register():
    ...
    if not all([phone, username, password, display_name]):
...
def logout():
    user_id = session.get('user_id')
    if user_id:
        execute_sql("UPDATE users SET status = 'offline', last_seen = ? WHERE id = ?", (int(time.time()), user_id))
    session.clear()
...
def get_messages(user, chat_type, chat_id):
    rows = query_all("SELECT m.id, m.sender_id, u.display_name as sender_name, m.content, m.timestamp FROM messages m JOIN users u ON m.sender_id = u.id WHERE m.chat_type = ? AND m.chat_id = ? ORDER BY m.timestamp DESC LIMIT 100", (chat_type, chat_id))
    rows.reverse()
...
def send_message(user):
    ...
    to_user_id = data.get('to_user_id')  # для приватных чатов
    if not content:
        return jsonify({'error': 'Empty message'}), 400
    if chat_type == 'private':
        if not to_user_id:
            return jsonify({'error': 'Missing recipient'}), 400
        # Вычисляем единый chat_id для хранения
        chat_id = min(user['id'], to_user_id)
        recipient_id = to_user_id
    else:
        # Для групповых чатов (пока не реализованы)
        return jsonify({'error': 'Unsupported chat type'}), 400
    ...
    # Отправляем через сокет получателю
...
def update_profile(user):
    ...
    if display_name is not None:
        execute_sql("UPDATE users SET display_name = ? WHERE id = ?", (display_name, user['id']))
    if password:
        pwd_hash = generate_password_hash(password)
        execute_sql("UPDATE users SET password_hash = ? WHERE id = ?", (pwd_hash, user['id']))
    return jsonify({'status': 'ok'})
