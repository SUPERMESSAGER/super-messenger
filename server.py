...
# ---------- SQLite для пользователей и сообщений ----------
...
def init_db():
    ...
    # Таблица сообщений (постоянное хранение)
    ...
    # Добавляем предустановленных пользователей (если их нет)
    # Админ 123 и обычный 321
    users_to_add = [
        ('123', '123', '123', 1),   # is_admin = 1
        ('321', '321', '321', 0)
    ]
    for username, display_name, password, is_admin in users_to_add:
        c.execute("SELECT id FROM users WHERE username = ?", (username,))
        if not c.fetchone():
            pwd_hash = generate_password_hash(password)
            c.execute("INSERT INTO users (username, display_name, password_hash, is_admin, created_at) VALUES (?, ?, ?, ?, ?)",
                      (username, display_name, pwd_hash, is_admin, int(time.time())))
    conn.commit()
    conn.close()
...
# ---------- HTML (встроен, основан на рабочем коде) ----------
@app.route('/')
def index():
    return '''<!DOCTYPE html>
<html lang="ru">
<head>
    ...
    <title>ALEX — мессенджер с админкой</title>
    <style>
...
<script>
    ...
    // Админ-панель (работает через отдельные API)
    async function openAdminPanel() {
        ...
        let html = `<div class="admin-panel"><div class="admin-panel-content">
            ...
            <table> hilab<th>ID</th><th>Username</th><th>Имя</th><th>Админ</th><th>Бан</th><th>Действия</th></tr>`;
        for (let u of users) {
            html += `<tr>
                <td>${u.id}</td>
        ...
        html += `</table><hr><h4>Заявки на регистрацию</h4><tr><tr><th>Username</th><th>Имя</th><th>Дата</th><th>Действие</th></tr>`;
        ...
        html += `</table><hr><h4>Все сообщения</h4><tr><tr><th>От</th><th>Кому</th><th>Текст</th><th>Время</th></tr>`;
...
# ---------- API маршруты (сохранены все старые + добавлены админские) ----------
...
def admin_sync_users(user):
    # Читаем users.txt и синхронизируем
