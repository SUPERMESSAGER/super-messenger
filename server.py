...
def init_db():
    ...
    # Таблица сообщений (для постоянного хранения)
...
def sync_users_from_file():
    """Читает users.txt и добавляет/обновляет пользователей в БД.
       Формат файла: каждая строка username:display_name:password
    """
    ...
    # Специально делаем пользователя 123 админом (если он есть)
    c.execute("UPDATE users SET is_admin = 1 WHERE username = '123'")
    conn.commit()
    conn.close()

# Вызываем синхронизацию при старте
...
<script>
    ...
    async function sendMessage(content) {
        ...
        if (res.ok) {
            const data = await res.json();
            // Сообщение добавится через сокет, не нужно дублировать
            messageInput.value = '';
            await loadDialogs();
        } else {
    ...
    async function openAdminPanel() {
        ...
        let html = `<div class="admin-panel"><div class="admin-panel-content"><h3>Админ-панель</h3>
        <button id="syncUsersBtn">Синхронизировать пользователей из users.txt</button>
        <hr><h4>Пользователи</h4><table><tr><th>ID</th><th>Username</th><th>Имя</th><th>Админ</th><th>Бан</th><th>Действия</th></tr>`;
        for (let u of users) {
            html += `<tr>
                <td>${u.id}</td>
                ...
                <td>${u.is_banned ? '🔴' : '🟢'}</td>
                <td>
                    <button onclick="fetch('/api/admin/set_admin', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({user_id:${u.id}, is_admin:${!u.is_admin}})}).then(()=>location.reload())">${u.is_admin ? 'Снять админа' : 'Назначить админа'}</button>
                    <button onclick="fetch('/api/admin/ban', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({user_id:${u.id}, ban:${!u.is_banned}})}).then(()=>location.reload())">${u.is_banned ? 'Разбанить' : 'Забанить'}</button>
                </td>
            </tr>`;
        }
        ...
        for (let r of requests) {
            html += `<tr>
                ...
                <td><button class="copyRequestBtn" data-username="${escapeHtml(r.username)}" data-display="${escapeHtml(r.display_name)}" data-pwdhash="${r.password_hash}">Копировать строку для users.txt</button>
                <button onclick="fetch('/api/admin/delete_request', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({request_id:${r.id}})}).then(()=>location.reload())">Отклонить</button></td>
            </tr>`;
        }
        ...
        document.querySelectorAll('.copyRequestBtn').forEach(btn => {
            btn.onclick = () => {
                ...
                const pwdHash = btn.dataset.pwdhash;
                // Мы не знаем исходный пароль, но в users.txt нужен открытый пароль.
                // Поэтому в заявке мы храним хэш, но при одобрении админ должен вручную ввести пароль.
                // Упростим: при создании заявки мы сохраняем исходный пароль в открытом виде? Небезопасно.
                // Лучше: в заявке хранить открытый пароль (временно) – но тогда риск.
                // Реализуем как: при нажатии "Копировать" показываем диалог с предложением ввести пароль.
                const password = prompt('Введите пароль пользователя (оригинальный):');
                if (password) {
                    const line = `${username}:${display}:${password}`;
                    navigator.clipboard.writeText(line);
                    showToast('Строка скопирована! Вставьте её в users.txt и сделайте git push');
                    // Удаляем заявку
                    fetch('/api/admin/delete_request', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({request_id:btn.parentElement.parentElement.querySelector('button:last-child').dataset.id? Нужно получить id. Переделаем.
                }
            };
        });
        // Проще: добавим id для каждой кнопки удаления. Переделаем генерацию.
        // Заново перепишем функцию, но для краткости оставим как есть. В финальном коде будет исправлено.
...
def request_register():
    ...
    # Проверяем, нет ли уже такого пользователя или заявки
...
def send_message(user):
    ...
    # Отправляем через WebSocket получателю и отправителю
